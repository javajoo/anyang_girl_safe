...
public class CommonUtil
{
  ...
  public static String removeInvalidXMLCharacters(String s)
  {
    ...
    while (i < s.length())
    {
      ...
      if ((codePoint == 9) || (codePoint == 10) || (codePoint == 13) || (codePoint == 56353) || ((codePoint >= 32) && (codePoint <= 55295)) || 
  ...
  public static String getClientIp(HttpServletRequest request)
	{
		//http헤더를 읽어와 프록시 서버, L4, 캐싱 서버, http서버용 WAS connector등으로 구성된 환경에서 클라이언트 IP를 문제없이 얻어오기
		//http헤더는 프록시 서버, L4, 캐싱서버 등 제품별로 헤더정보가 다를 수 있으므로 만약 아래에 기입되지 않은 별도의 헤더를 사용하는 제품이 있다면 헤더 정보를 추가 등록하여 문제를 피할 수 있음
